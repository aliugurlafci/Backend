...
// userIds are the numeric-string ids the auth seed assigns to the `user` rows
// (admin=1, manager=2, rep=3, accountant=4), so the dev actor's id, the real
// login user id and the demo records' ownerId all line up.
export const DEMO_USERS: Record<string, Principal> = {
  admin: { userId: "1", displayName: "Avery Admin", email: "avery@acme.test", roles: ["admin"] },
  manager: { userId: "2", displayName: "Morgan Manager", email: "morgan@acme.test", roles: ["sales_manager"] },
  rep: { userId: "3", displayName: "Riley Rep", email: "riley@acme.test", roles: ["sales_rep"] },
  accountant: { userId: "4", displayName: "Casey Accountant", email: "casey@acme.test", roles: ["accountant"] },
};
